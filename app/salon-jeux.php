    ...
    <div class="container mx-auto max-w-7xl">
        ...
        <div class="grid grid-cols-3 gap-4 max-w-2xl mx-auto mb-12">
            <div class="glass-card rounded-2xl p-5 modern-border border-2 border-white/10 text-center">
                <div class="text-3xl font-bold text-accent mb-1"><?php echo $stats['total_jeux']; ?></div>
                <div class="text-xs text-light-80 flex items-center justify-center gap-2"><i class="fas fa-gamepad text-accent"></i>Jeux</div>
            </div>
            <div class="glass-card rounded-2xl p-5 modern-border border-2 border-white/10 text-center">
                <div class="text-3xl font-bold text-yellow-400 mb-1"><?php echo $stats['total_candidats']; ?></div>
                <div class="text-xs text-light-80 flex items-center justify-center gap-2"><i class="fas fa-star text-yellow-400"></i>Candidats</div>
            </div>
            <div class="glass-card rounded-2xl p-5 modern-border border-2 border-white/10 text-center">
                <div class="text-3xl font-bold text-purple-400 mb-1"><?php echo $stats['total_comments']; ?></div>
                <div class="text-xs text-light-80 flex items-center justify-center gap-2"><i class="fas fa-comments text-purple-400"></i>Commentaires</div>
            </div>
        </div>
                ...
                <div class="glass-card rounded-3xl p-5 modern-border border-2 border-white/10">
                    ...
                    <div class="space-y-3">
                        <?php foreach ($popular_jeux as $i => $pjeu): ?>
                            <a href="jeu-campagne.php?id=<?php echo $pjeu['id_jeu']; ?>" class="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors group border border-white/10 hover:border-accent/30">
                                <span class="w-8 h-8 rounded-full bg-yellow-500/20 flex items-center justify-center text-xs font-bold text-yellow-400 border border-yellow-500/30"><?php echo $i + 1; ?></span>
                                <div class="w-8 h-8 rounded-lg overflow-hidden bg-white/5 flex-shrink-0 border border-white/10">
                                    ...
                                    <?php else: ?>
                                        <div class="w-full h-full flex items-center justify-center"><i class="fas fa-gamepad text-light/20 text-xs"></i></div>
                                    <?php endif; ?>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-light text-sm font-medium truncate group-hover:text-accent transition-colors"><?php echo htmlspecialchars($pjeu['titre']); ?></p>
                                    <p class="text-light-80 text-xs"><i class="fas fa-vote-yea mr-1"></i><?php echo $pjeu['nb_votes']; ?> votes</p>
                                </div>
                            </a>
                        <?php endforeach; ?>
                    </div>
                ...
                <div class="glass-card rounded-3xl p-5 modern-border border-2 border-white/10">
                    ...
                    <div class="space-y-3">
                        <?php foreach ($active_candidats as $candidat): ?>
                            <a href="jeu-campagne.php?id=<?php echo $candidat['id_jeu']; ?>" class="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors group border border-white/10 hover:border-yellow-500/30">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-light-80 text-xs truncate"><?php echo htmlspecialchars(substr($candidat['jeu_titre'], 0, 20)); ?></p>
                                </div>
                                <span class="text-xs text-light-80 flex-shrink-0"><i class="fas fa-comments mr-1"></i><?php echo $candidat['nb_posts']; ?></span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <?php if (!empty($search)): ?>
                    <div class="mb-6 flex items-center justify-between">
                        <p class="text-light-80">
                            <span class="text-light font-medium"><?php echo count($jeux); ?></span> résultat(s) pour "<span class="text-accent font-medium"><?php echo $search; ?></span>"
                        </p>
                            ...
                            <a href="jeu-campagne.php?id=<?php echo $jeu['id_jeu']; ?>" class="group">
                                <div class="glass-card rounded-2xl overflow-hidden modern-border border-2 border-white/10 hover:border-accent/50 transition-all duration-300 h-full flex flex-col">
                                    ...
                                    <div class="p-5 flex flex-col flex-1">
                                        <h3 class="text-lg font-bold text-light mb-1 group-hover:text-accent transition-colors truncate"><?php echo htmlspecialchars($jeu['titre']); ?></h3>
                                        <?php if ($jeu['editeur']): ?>
                                            <p class="text-light-80 text-sm mb-3 truncate"><?php echo htmlspecialchars($jeu['editeur']); ?></p>
                                        <?php endif; ?>
                                        
                                        <div class="flex items-center justify-between pt-3 mt-auto border-t border-white/10">
                                            <span class="text-xs text-light-80">
                                                <?php if ($jeu['candidat_nom']): ?>
                                                    <i class="fas fa-crown text-yellow-500 mr-1"></i><span class="font-medium text-yellow-400">Avec candidat</span>
                                                <?php else: ?>
                                                    <i class="fas fa-gamepad mr-1"></i><span>Sans candidat</span>
                                                <?php endif; ?>
                                            </span>
