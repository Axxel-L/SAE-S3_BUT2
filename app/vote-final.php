...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'vote_final') {
    ...
    try {
        ...
        if ($event['statut'] !== 'ouvert_final') throw new Exception("Le vote final n'est pas ouvert.");
        ...
        if ($stmt->rowCount() === 0) throw new Exception("Vous n'êtes pas inscrit.");
        ...
        $stmt->execute([$id_utilisateur, $id_evenement]);
        if ($stmt->rowCount() > 0) throw new Exception("Vous avez déjà voté pour la finale !");
        
        // Vérifier que le jeu est finaliste
        ...
        if ($stmt->rowCount() === 0) throw new Exception("Ce jeu n'est pas finaliste.");
...
// Config statuts
$statut_config = [
    'preparation' => ['label' => 'Préparation', 'color' => 'yellow', 'icon' => 'fa-hourglass-start', 'can_vote_final' => false],
    'ouvert_categories' => ['label' => 'Vote Catégories', 'color' => 'green', 'icon' => 'fa-vote-yea', 'can_vote_final' => false],
    'ferme_categories' => ['label' => 'Attente Vote Final', 'color' => 'blue', 'icon' => 'fa-pause-circle', 'can_vote_final' => false],
    'ouvert_final' => ['label' => 'Vote Final', 'color' => 'purple', 'icon' => 'fa-crown', 'can_vote_final' => true],
    'cloture' => ['label' => 'Clôturé', 'color' => 'red', 'icon' => 'fa-times-circle', 'can_vote_final' => false]
];
    ...
    <section class="py-20 px-6 min-h-screen">
        <div class="container mx-auto max-w-6xl">
            ...
            <?php else: ?>
                ...
                ?>
                    <div class="glass-card rounded-4xl p-8 mb-8 modern-border <?php echo $status['can_vote_final'] ? 'glow-animation' : ''; ?>">
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div class="p-4 rounded-xl bg-white/5 border border-white/10">
                                    <div class="flex items-center gap-2 mb-2">
                                        <i class="fas fa-layer-group text-light/60"></i>
                                        <span class="font-bold text-light/80">Phase 1 : Vote Catégories</span>
                        ...
                        <?php if (!$isRegistered): ?>
                            <div class="p-6 rounded-2xl bg-orange-500/10 border border-orange-500/30 text-center">
                                ...
                                <p class="text-orange-400 text-lg mb-4">Vous devez vous inscrire pour voter.</p>
                        ...
                        <?php elseif (!$status['can_vote_final']): ?>
                            <div class="p-8 rounded-2xl bg-purple-500/10 border border-purple-500/30 text-center">
                                <?php if ($event['statut'] === 'ouvert_categories'): ?>
                                    ...
                                    <a href="vote.php" class="inline-flex items-center gap-2 px-6 py-3 mt-4 rounded-2xl bg-green-500 text-dark font-bold hover:bg-green-400 transition-colors">
                            ...
                            <?php if (!empty($finalistes) && $event['statut'] !== 'ouvert_categories'): ?>
                                <div class="mt-8">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        <?php foreach ($finalistes as $jeu): ?>
                                            <div class="glass-card rounded-2xl p-4 modern-border opacity-75">
                                                <?php if ($jeu['image']): ?>
                                                    <div class="mb-3 rounded-xl overflow-hidden h-32 bg-black/50">
                                                        <img src="<?php echo htmlspecialchars($jeu['image']); ?>" class="w-full h-full object-cover">
                                                    </div>
                                                <?php endif; ?>
                                            ...
                                            <div class="glass-card rounded-3xl p-6 h-full modern-border hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/20 transition-all duration-300 <?php echo $dejaVoteFinal ? 'opacity-50 cursor-not-allowed' : ''; ?>">
                                                <?php if ($jeu['image']): ?>
                                                    <div class="mb-4 rounded-xl overflow-hidden h-40 bg-black/50">
                                                        <img src="<?php echo htmlspecialchars($jeu['image']); ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                                    </div>
                                                <?php else: ?>
            ...
            <div class="text-center mt-12">
                <a href="vote.php" class="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-white/5 border border-white/10 hover:border-accent/50 transition-colors">
                    <i class="fas fa-arrow-left"></i>Retour au vote par catégorie
                </a>
            </div>
        </div>
    </section>
